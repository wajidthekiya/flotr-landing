...
export default function DestinationFlow() {
  ...
  return (
    <section className="py-32 bg-[var(--color-deep)] overflow-hidden relative">
      <div className="absolute top-0 right-0 w-[50vw] h-[50vw] bg-[var(--color-accent)] rounded-full mix-blend-screen filter blur-[150px] opacity-10 pointer-events-none"></div>
      ...
      <div className="px-6 md:px-20 mb-12 flex flex-col md:flex-row justify-between items-end gap-6">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="flex items-center gap-3 text-[var(--color-accent)] font-body font-medium"
        >
          <span>Swipe to explore</span>
          <FaLocationArrow className="animate-bounce" />
        </motion.div>
      </div>
        ...
        {destinations.map((dest) => (
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8 z-20 flex flex-col justify-end h-full bg-gradient-to-t from-[var(--color-deep)] via-[var(--color-deep)]/40 to-transparent">
              <h3 className="font-heading text-white text-5xl md:text-6xl tracking-wider mb-4 group-hover:text-[var(--color-accent)] transition-colors duration-300">{dest.name}</h3>
              <button className="self-start bg-[var(--color-primary)] text-white px-8 py-3 rounded-full font-body font-medium hover:bg-[var(--color-accent)] hover:text-[var(--color-deep)] transition-colors shadow-[0_0_20px_rgba(151,123,242,0.4)]">
                Explore Drops
              </button>
            </div>
          </Tilt>
