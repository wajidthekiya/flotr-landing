...
export default function CommunityEnergy() {
  return (
    <section className="relative py-40 w-full flex items-center justify-center overflow-hidden bg-[var(--color-deep)]">
      ...
      <div className="relative z-10 text-center text-[var(--color-text)] px-6 max-w-5xl">
        <motion.div
          initial={{ opacity: 0, scale: 0.8 }}
          whileInView={{ opacity: 1, scale: 1 }}
          viewport={{ once: true }}
          className="inline-block mb-8 bg-[var(--color-surface)] backdrop-blur-md border border-[var(--color-border)] px-6 py-2 rounded-full font-body font-bold text-sm tracking-widest uppercase text-[var(--color-primary)]"
        >
          Join the Movement
        </motion.div>
        ...
        <motion.p 
          ...
          className="font-body text-xl md:text-3xl text-[var(--color-text)]/80 mt-10 max-w-2xl mx-auto leading-relaxed"
        >
        ...
        <motion.button
          ...
          className="mt-12 bg-[var(--color-accent)] text-[#090514] px-12 py-5 rounded-full font-bold text-xl hover:scale-105 hover:shadow-[0_0_40px_rgba(214,244,88,0.6)] transition-all duration-300"
        >
          Find Your Crew
        </motion.button>
      </div>
