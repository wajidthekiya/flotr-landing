...
export default function MovementTypography() {
  ...
  return (
    <section className="py-40 bg-[var(--color-deep)] relative overflow-hidden group/section">
      ...
      <div className="max-w-7xl mx-auto px-6 flex flex-col items-center text-center space-y-4 md:space-y-8">
        {words.map((word, index) => (
          ...
          >
            ...
            {/* Typography */}
            <h2 className={`font-heading text-[4.5rem] md:text-[12rem] lg:text-[15rem] leading-[0.85] tracking-tighter transition-all duration-300 relative z-10 ${word.color}`}>
              {word.text}
            </h2>
          </motion.div>
        ))}
      ...
      >
        <div className="inline-block bg-[var(--color-surface)] backdrop-blur-md border border-[var(--color-border)] px-8 py-4 rounded-full">
          <p className="font-body text-xl md:text-2xl text-[var(--color-text)]/80 max-w-md mx-auto leading-relaxed">
